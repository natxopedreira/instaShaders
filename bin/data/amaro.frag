...
uniform sampler2DRect u_Texture1;
...
void main()
{
     ...
     vec3 _filter = texture(u_Texture1, st).rgb;
     ...
     //add blue
     vec3 rb_result = vec3(bcs_result.r, bcs_result.g*1.03, bcs_result.b);
     
     //add filter (overlay blending)
     vec3 after_filter = mix(rb_result, ovelayBlender(rb_result, _filter), 0.6);
